...
class GeneralInfo(BaseModel):
    ...
    noticeType: str = Field(
        max_length=100, description="공고 종류. 예: 물품, 공사, 용역, 외자, 비축물자 등"
    )
    noticeNo: str = Field(max_length=100, description="공고번호 또는 구매결의번호 (예: R26BK00000000-000)")
    refNo: str = Field(
    ...
    agency: str = Field(default="", max_length=200, description="공고기관명 (예: 조달청)")
    ...
    contractForm: str = Field(
        default="", max_length=100, description="계약형태 (예: 단가, 총액, 회차)"
    )
    bidMethod: Literal["전자입찰", "수기입찰"] = Field(
        default="", max_length=100, description="입찰방식 (예: 전자입찰, 수기입찰)"
    )
    stockType: str = Field(
        default="",
        max_length=100,
...
class Item(BaseModel):
    ...
    unit: str = Field(default="", max_length=50, description="단위 (예: 박스, EA, kg)")
...
class Progress(BaseModel):
    ...
    stepName: str = Field(
        max_length=100, description="진행 단계명 (예: 공고, 입찰등록, 입찰마감, 개찰, 낙찰)"
    )
...
class Status(BaseModel):
    ...
    result: str = Field(
        default="", max_length=50, description="투찰 결과 (예: 낙찰, 낙찰탈락, 무효)"
    )
